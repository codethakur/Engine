            ...
            component->onClick = [this]()
                {
                    ...
                    m_gui.SetState(GameState::Playing);
                    if (m_3DRoot)
                    {
...
void Game::Update(float deltaTime)
{
    ...
    {
        if (m_gui.GetState()  == GameState::Playing && m_3DRoot && m_3DRoot->IsActive())
        {
            //engine.GetUIInputSystem().GetCanvas()->SetActive(true);
            m_gui.SetState(GameState::Paused);
            engine.SetCursorEnabled(true);
           // m_3DRoot->SetActive(false);
        }
        else if (m_gui.GetState() == GameState::Paused)
        {
            m_gui.SetState(GameState::Playing);
            engine.SetCursorEnabled(false);
        }
    ...
    if (m_gui.GetState() == GameState::Playing)
        m_gui.RenderHUD();
    else if (m_gui.GetState() == GameState::Paused)
        m_gui.RenderPauseMenu();

    if (m_gui.ShowSettings() && m_gui.GetState() == GameState::Paused)
        m_gui.RenderSettingsWindow();
