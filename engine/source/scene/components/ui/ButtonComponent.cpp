...
namespace eng
{
    ...
    void ButtonComponent::Render(CanvasComponent* canvas)
    {
        ...
        auto rectTranform = GetOwner()->GetComponent<RectTransformComponent>();
        if(!rectTranform)
        {
            return;
        }
        auto ownerPosition = rectTranform->GetScreenPosition();
        ownerPosition -= rectTranform->GetSize() * rectTranform->GetPivot();

        canvas->DrawRect(ownerPosition, ownerPosition + rectTranform->GetSize(), *m_currentColor);
    }

    bool ButtonComponent::HitTest(const glm::vec2& position) 
    {
        auto rectTranform = GetOwner()->GetComponent<RectTransformComponent>();
        if(!rectTranform)
        {
        ...
        auto p1 = ownerPosition-rectTranform->GetSize() * rectTranform->GetPivot();
        auto p2 = p1 +rectTranform->GetSize();
        return (p1.x <= position.x && p2.x >= position.x && p1.y <= position.y && p2.y >= position.y);
    }
