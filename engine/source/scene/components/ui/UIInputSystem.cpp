...
namespace eng
{
    ...
    void UIInputSystem::Update(float deltaTime)
    {
       ...
       auto mousePos = input.GetMousePositionCurrent();
        ...
        glfwGetWindowSize(Engine::GetInstance().GetWindow(), &winWidth, &winHeight); 
        mousePos.y = static_cast<float>(winHeight) - mousePos.y;
    ...
    void UIInputSystem::Update(float deltaTime)
    {
        ...
        // Fix Mac Retina 2x scaling
        int fbWidth, fbHeight;
        int winWidth, winHeight;
        glfwGetFramebufferSize(Engine::GetInstance().GetWindow(), &fbWidth, &fbHeight);
        glfwGetWindowSize(Engine::GetInstance().GetWindow(), &winWidth, &winHeight);
        float scaleX = (float)fbWidth / (float)winWidth;
        float scaleY = (float)fbHeight / (float)winHeight;
        mousePos.x *= scaleX;
        mousePos.y *= scaleY;
        mousePos.y = (float)fbHeight - mousePos.y;
    ...
    std::vector<UIElementComponent*> UIInputSystemCollectUI(CanvasComponent* canvas)
    {
